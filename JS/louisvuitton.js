import { LouisvuittonData } from "./productData.js";
...
LouisvuittonData.slice(0, 9).forEach((it) => {
  loadProductFromDataTab1(it);
});
LouisvuittonData.slice(9).forEach((it) => {
...
sortSelect.addEventListener("change", function () {
  ...
  if (sortSelect.value === "Mới nhất") {
    ...
    let newLouisvuittonData = LouisvuittonData.sort((a, b) => (a.name > b.name ? 1 : -1));
    newLouisvuittonData.forEach((item) => {
    ...
    let newLouisvuittonData = LouisvuittonData.sort((a, b) => (a.price > b.price ? 1 : -1));
    newLouisvuittonData.forEach((item) => {
      loadProductFromDataTab1(item);
    });
    handleProductDetail();
