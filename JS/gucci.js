import { GucciData } from "./productData.js";
...
GucciData.slice(0, 9).forEach((it) => {
  loadProductFromDataTab1(it);
});
GucciData.slice(9).forEach((it) => {
...
sortSelect.addEventListener("change", function () {
  ...
  if (sortSelect.value === "Mới nhất") {
    ...
    let newGucciData = GucciData.sort((a, b) => (a.name > b.name ? 1 : -1));
    newGucciData.forEach((item) => {
    ...
    let newGucciData = GucciData.sort((a, b) => (a.price > b.price ? 1 : -1));
    newGucciData.forEach((item) => {
      loadProductFromDataTab1(item);
    });
    handleProductDetail();
