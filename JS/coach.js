import { CoachData } from "./productData.js";
...
CoachData.slice(0, 9).forEach((it) => {
  loadProductFromDataTab1(it);
});
CoachData.slice(9).forEach((it) => {
...
sortSelect.addEventListener("change", function () {
  ...
  if (sortSelect.value === "Mới nhất") {
    ...
    let newCoachData = CoachData.sort((a, b) => (a.name > b.name ? 1 : -1));
    newCoachData.forEach((item) => {
    ...
    let newCoachData = CoachData.sort((a, b) => (a.price > b.price ? 1 : -1));
    newCoachData.forEach((item) => {
      loadProductFromDataTab1(item);
    });
    handleProductDetail();
