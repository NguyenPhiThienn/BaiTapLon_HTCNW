import { DiorData } from "./productData.js";
...
DiorData.slice(0, 9).forEach((it) => {
  loadProductFromDataTab1(it);
});
DiorData.slice(9).forEach((it) => {
...
sortSelect.addEventListener("change", function () {
  ...
  if (sortSelect.value === "Mới nhất") {
    ...
    let newDiorData = DiorData.sort((a, b) => (a.name > b.name ? 1 : -1));
    newDiorData.forEach((item) => {
    ...
    let newDiorData = DiorData.sort((a, b) => (a.price > b.price ? 1 : -1));
    newDiorData.forEach((item) => {
      loadProductFromDataTab1(item);
    });
    handleProductDetail();
