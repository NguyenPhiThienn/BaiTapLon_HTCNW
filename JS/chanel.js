import { ChanelData } from "./productData.js";
...
ChanelData.slice(0, 9).forEach((it) => {
  loadProductFromDataTab1(it);
});
ChanelData.slice(9).forEach((it) => {
...
sortSelect.addEventListener("change", function () {
  ...
  if (sortSelect.value === "Mới nhất") {
    ...
    let newChanelData = ChanelData.sort((a, b) => (a.name > b.name ? 1 : -1));
    newChanelData.forEach((item) => {
    ...
    let newChanelData = ChanelData.sort((a, b) => (a.price > b.price ? 1 : -1));
    newChanelData.forEach((item) => {
      loadProductFromDataTab1(item);
    });
    handleProductDetail();
